...
const std::string testDataPath = "../vision_transformer";  // 用于推理
// const std::string testImagePath = dataPath + "/test/roses_01.jpg";  // 用于推理
const int         classesNum = 1000;
...
void imagePostProcess(std::string file_name, float* outputs)
{
    ...
    std::cout << " category is : " << maxValIdx;
    std::cout << ", prob is : ";
...
int run()
{
    ...
    // inference
    std::string testImagePath = "../banana.jpeg";
    ...
    imagePostProcess("banana.jpeg", outputData);
    auto end = std::chrono::system_clock::now();

    int total_cost = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
    std::cout << "Inference cost is: " << total_cost << " ms" << std::endl;
